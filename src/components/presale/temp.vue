        ...
        <div id="stages">
          ...
          <v-row :class="isMobileDevice ? 'mt-2 ml-2 mr-2' : 'mt-2 mb-16'" >
            <v-col cols="12" md="12" :align="'center'">
              ...
              <div class="text-body-1 text-white">EMAS tokens will be claimable 14 days after the presale finishes and listing will take place a further 6 days after the claiming</div>
  ...
  export default {
    ...
    methods: {
      ...
      async buyWithEthContract () {
        ...
        try {
          this.presaleContract.methods.buyWithEth(`${3}`, `${tokens}`).send({from: this.getUser.accounts[0], value: `${eth}`}, function(error, transactionHash){
            //
          });
        } catch(err) {
          console.log(err)
